...
describe('AddressService', () => {
  ...
  it('should return error if exception in userService', async () => {
    jest.spyOn(userService, 'findUserById').mockRejectedValue(new NotFoundException('User not found'));
    
    expect(service.createAddress(createAddressMock, userEntityMock.id)).rejects.toThrow(NotFoundException);
  });

  it('should return error if exception in cityService', async () => {
    jest.spyOn(cityService, 'findCityById').mockRejectedValue(new NotFoundException('User not found'));
  ...
  it('should return not found if address not exist', async () => {
